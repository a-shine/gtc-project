...
def load_data(exp, dirs, params):
	...
	if params['data_type'] == 'lab':
		...
		filename = dirs['data']+params['case_study']+'/'+exp+'.'+params['file_format']
		...
		Y = np.array([ShearStressobs, NormalStressobs, ecDispobs, ShearStrainobs]).T
	elif params['data_type'] == 'nature':
		f = h5py.File(dirs['data']+params['case_study']+'.'+params['file_format'],'r')
		...
		sigman0 = np.loadtxt(dirs['data']+params['case_study']+'/sigman0.txt')
		vl      = params['vl']
		if vl is None:
		...
		data_e = np.loadtxt(dirs['data']+params['case_study']+'pgc5_E_wtrend.data')
		data_n = np.loadtxt(dirs['data']+params['case_study']+'pgc5_N_wtrend.data')
